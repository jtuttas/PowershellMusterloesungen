...
foreach ($s in $schueler) {
    ...
    $s=New-Coursevoting -id $s.id -course1 $kurs1.id -course2 $kurs2.id -course3 $kurs3.id
}
...
foreach ($k in $kurse) {
    ...
    [object[]]$schueler = Get-Coursevoting -id $k.id -priority 1     
    ...
    for ($i=0;$i -lt $m;$i++) {
        ...
        # Schülers als gewählt amrkieren
        Stop-Coursevoting -id $schueler[$i].id 
...
foreach ($k in $kurse) {
    # Ermitteln der Schüler, die diesen Kurs als Zweiwunsch hatten
    [object[]]$schueler = Get-Coursevoting -id $k.id -priority 2 
    $nk = Get-Coursemember $k.id | measure 
    Write-Host "Im Kurs "$k.TITEL" sind bisher "$nk.Count" Schüler"
    $m=25-$nk.Count;
    if ($schueler.Count -lt $m) {
