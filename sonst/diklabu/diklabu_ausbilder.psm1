...
function Find-Instructor
{
    ...
    Process
    {
        foreach ($n in $name) {
            ...
            $r=Invoke-RestMethod -Method Get -Uri ($uri+"admin/ausbilder/find/"+$n) -Headers $headers  
            return $r;
        }
...
function Get-Instructor
{
    ...
    Process
    {
        foreach ($id in $ausbilderId) {
            ...
            $headers["content-Type"]="application/json;charset=iso-8859-1"
            $headers["auth_token"]=$global:auth_token;
            $r=Invoke-RestMethod -Method Get -Uri ($uri+"admin/ausbilder/"+$id) -Headers $headers  
            return $r;
        }
...
function Set-Instructor
{
    ...
    Process
    {
        foreach ($id in $ausbilderId) {
            ...
            $headers["content-Type"]="application/json;charset=iso-8859-1"
            $headers["auth_token"]=$global:auth_token;
            $r=Invoke-RestMethod -Method Post -Uri ($uri+"admin/ausbilder/"+$id) -Headers $headers -Body (ConvertTo-Json $ausbilder)
            return $r;
        }
    }
    End
...
function New-Instructor
{
    ...
    Begin
    {
            ...
            $headers["content-Type"]="application/json;charset=iso-8859-1"
            $headers["auth_token"]=$global:auth_token;
            $r=Invoke-RestMethod -Method Post -Uri ($uri+"admin/ausbilder/") -Headers $headers -Body (ConvertTo-Json $ausbilder)
            return $r;
...
function Delete-Instructor
{
    ...
    Process
    {
        foreach ($id in $ausbilderId) {        
            ...
            $headers["content-Type"]="application/json;charset=iso-8859-1"
            $headers["auth_token"]=$global:auth_token;
            $r=Invoke-RestMethod -Method Delete -Uri ($uri+"admin/ausbilder/"+$id) -Headers $headers 
            return $r;
        }
    }
    End
